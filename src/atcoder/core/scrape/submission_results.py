...
def scrape_task_ids(html: bytes) -> typing.List[str]:
    return unwrap(_scrape_html_options(html, "select-task"))


def scrape_language_categories(html: bytes) -> typing.List[str]:
    return unwrap(_scrape_html_options(html, "select-language"))


def scrape_submission_statuses(html: bytes) -> typing.List[str]:
    return unwrap(_scrape_html_options(html, "select-status"))
...
async def _scrape_submission_row(row: bs4.element.Tag) -> SubmissionResult:
...
async def scrape_submissions(
    ...
    #     yield await scrape_submission(row)

    return [
        await _scrape_submission_row(row) for row in table.tbody.find_all("tr")
    ]
